...
-- vicious.register(aptwidget, vicious.widgets.pkg, function(widget, args)
--     return args[1]
--   end, 1801, "Ubuntu")
-- --
-- -- Buttons
-- function popup_apt()
--   local apt_updates = ""
--   local f = io.popen("apt list --upgradeable | grep -v Listing | wc -l")
--   if f then
--     apt_updates = f:read("*a"):match("^%d+\n$")
--   end
--   f:close()
--   if not apt_updates then
--     apt_updates = "System is up to date"
--   end
--   naughty.notify { text = apt_updates }
-- end
-- aptwidget:buttons(awful.util.table.join(awful.button({ }, 1, popup_apt)))
-- apticon:buttons(aptwidget:buttons())
-- End APT }}}
--
...
local function setvolicon(muted)
    if muted then
